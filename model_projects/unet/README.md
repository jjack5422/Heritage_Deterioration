...
```bash
crackseg_env/bin/python model_projects/unet/src/train.py \
...
```text
model_projects/unet/runs/<experiment-id>/
...
```text
model_projects/unet/runs/<experiment-id>/5fold/<expert>/foldN/tensorboard/images/
├── manifest.csv
├── loss_curve.png
├── best/*.png
└── worst/*.png
...
crackseg_env/bin/python ~/.codex/skills/training-output-reporting/scripts/build_training_report.py \
  --run-dir model_projects/unet/runs/<experiment-id>/5fold/<expert>/foldN
```
...
```bash
crackseg_env/bin/python model_projects/unet/reporting/backfill.py \
  --legacy-root model_projects/unet/runs/<experiment-id>/info/legacy \
...
```bash
RUN=model_projects/unet/runs/<experiment-id>/5fold/crack/fold0
DATA=datasets/dataset_clean_v2

crackseg_env/bin/python model_projects/unet/src/predict_full.py \
...
crackseg_env/bin/python model_projects/unet/src/thresholding.py \
...
```bash
uv pip install --python crackseg_env/bin/python -r model_projects/unet/requirements-gui.txt
```

直接開啟 validation threshold 瀏覽器，不必指定 checkpoint 或圖片：

```bash
crackseg_env/bin/python model_projects/unet/src/threshold_preview.py
```
...
`Export` 後輸出放在 `model_projects/unet/outputs/threshold_preview/`。
...
crackseg_env/bin/python model_projects/unet/src/predict_full.py \
  ...
  --out_dir model_projects/unet/outputs/crack_preview \
  --save_prob \
  --preview-threshold
```
