...
from crackseg_common.checkpoints import (
    load_trainable_state_dict,
    model_parameter_counts,
    trainable_state_dict,
)
from crackseg_common.binary_metrics import binary_summary, pixel_accuracy
from crackseg_common.binary_reporting import EpochReporter, RunLayout, append_log, finalize_reporting, save_qualitative_example, write_json
from crackseg_common.runtime import (
    autocast_context,
    batch_tensor,
    git_revision,
    package_versions,
...
from model_projects.sam3_adapter.loss_functions import weighted_bce_dice_loss
from model_projects.sam3_adapter.model import Sam3AdapterModel

PROJECT_ROOT = Path(__file__).resolve().parent
REPOSITORY_ROOT = PROJECT_ROOT.parents[1]
DEFAULT_MANIFEST_DIR = REPOSITORY_ROOT / "outputs" / "deterioration_statistics" / "transfer_512"
DEFAULT_CHECKPOINT = REPOSITORY_ROOT / "segment-anything-3" / "checkpoints" / "sam3.pt"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--model-input-size", type=int, default=DEFAULT_MODEL_INPUT_SIZE)
    ...
    parser.add_argument("--positive-weight", type=float, help="Override the expert default BCE positive weight")
    ...
    if args.model_input_size < 224:
        parser.error("model input size must be at least 224")
    if args.epochs <= 0:
    ...
    if not 0.0 <= args.dice_weight <= 1.0:
        parser.error("dice weight must be between 0 and 1")
...
def _loader(
    rows: Sequence[Mapping[str, Any]], *, train: bool, args: argparse.Namespace,
) -> DataLoader:
...
def _evaluate(
    ...
    with torch.no_grad():
        for batch in loader:
            images = batch_tensor(batch, "image", device)
            target = batch_tensor(batch, "target", device).long()
...
def train(args: argparse.Namespace, plan: ExpertDataPlan, device: torch.device) -> Path:
    ...
    test_loader = _loader(plan.test, train=False, args=args)
