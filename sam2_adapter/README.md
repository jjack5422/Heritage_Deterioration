...
完整訓練結束時還會呼叫
`$HOME/.codex/skills/training-output-reporting/scripts/` 內的三個 reporting
scripts。移機時必須一併複製目前工作站的
`~/.codex/skills/training-output-reporting/`，否則模型 epochs 能執行，
但 TensorBoard PNG 與 HTML dashboard 的最後匯出會失敗。
...
PYTHONPATH=. python -m sam2_adapter.train_adapter --help
```

正式訓練前，用實際 dataset 與 checkpoint 執行一次 optimizer-step smoke test：

```bash
PYTHONPATH=. python -m sam2_adapter.train_adapter \
  --folds 0 --smoke-test \
