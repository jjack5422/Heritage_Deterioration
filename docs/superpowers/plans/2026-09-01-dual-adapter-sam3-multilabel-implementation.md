# Dual-Adapter SAM3 Multi-label Segmentation 實作計畫
...
```text
2026-09-01_dual-adapter-sam3-multilabel-512_seed42
```
...
rg -n "1008|--prompt|zero.?shot|shared.?DPE|no.?DER|loss-only" \
  dual_adapter_sam3 -g '*.py' -g '*.yaml' -g '*.md'
```
